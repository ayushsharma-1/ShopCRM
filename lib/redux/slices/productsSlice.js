...
const initialState = {
  ...
  itemsPerPage: 12,
  isLoading: false,
  dataSource: null, // 'mongodb', 'json', or 'json-fallback'
};
...
const productsSlice = createSlice({
  ...
  reducers: {
    ...
    setProductsWithSource: (state, action) => {
    ...
    updateProductStock: (state, action) => {
      ...
      updatedProducts.forEach(updated => {
        ...
        if (index !== -1) {
          state.products[index] = { ...state.products[index], stock: updated.stock };
        }
        const filteredIndex = state.filteredProducts.findIndex(p => p.id === updated.id);
        if (filteredIndex !== -1) {
          state.filteredProducts[filteredIndex] = { ...state.filteredProducts[filteredIndex], stock: updated.stock };
        }
      });
    },
  },
...
export const fetchProducts = (type = 'products') => async (dispatch) => {
  try {
    ...
    if (result.success) {
      dispatch(setProductsWithSource({
        data: result.data,
    ...
    // Component should handle error and use JSON fallback if needed
  }
};
